...
<head>
    ...
    <style>
        ...
        @media (max-width: 576px) {
            ...
            .search-results-dropdown.dark .search-result-item {
            ...
            .search-results-dropdown.dark .search-result-item:hover {
            ...
            .search-results-dropdown.dark .search-result-item .preview {
                color: #aaa;
            }
            ...
            @keyframes highlight-animation {
                0% {
                    transform: translateY(0);
                    box-shadow: 0 0 0 rgba(76, 81, 191, 0);
                }
...
<body>
    <div class="container">
        ...
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div id="searchResults" class="search-results-dropdown position-absolute w-100 d-none"></div>
                </div>
            </div>
            <div class="card-body">
        ...
        <script>
            ...
            // First, remove any existing search results container if present
            $('#searchResults').remove();

            // Add container for search results if it doesn't exist
            $('#searchBox').wrap('<div class="search-container position-relative"></div>');
            $('#searchBox').after('<div id="searchResults" class="search-results-dropdown position-absolute w-100 d-none"></div>');

            // Add better styling that matches your app's design
            $('head').append(`
    <style>
        .search-container {
            position: relative;
        ...
        .dark-theme .search-result-item .preview {
            color: #aaa;
        }
        ...
        @keyframes highlight-animation {
            ...
            25% { transform: scale(1.02); box-shadow: 0 10px 25px rgba(76, 81, 191, 0.3); }
...
`);
            ...
            $('#searchBox').on('input', function() {
                ...
                searchTimeout = setTimeout(function() {
                    $.post('search_notes.php', {
                        ...
                        if (data.success && data.notes && data.notes.length > 0) {
                            ...
                            data.notes.forEach(note => {
                                ...
                                const title = note.title ? note.title.replace(/</g, '&lt;').replace(/>/g, '&gt;') : 'Untitled Note';
                                const content = note.content ? note.content.replace(/</g, '&lt;').replace(/>/g, '&gt;') : '';
                                const preview = content.substring(0, 60) + (content.length > 60 ? '...' : '');

                                resultsHtml += `
                        <div class="search-result-item" data-note-id="${note.note_id}">
                    ...
                    `;
                            ...
                            // Apply theme class if dark theme is active
                            if ($('body').hasClass('dark-theme') || $('html').hasClass('dark-theme')) {
                                searchResults.addClass('dark-theme');
                            }
                        ...
                        } else if (data.success && (!data.notes || data.notes.length === 0)) {
                            searchResults.html('<div class="no-results">No matching notes found</div>')
                                .removeClass('d-none');
                            ...
                            searchResults.html('<div class="no-results">Error searching notes</div>')
                                .removeClass('d-none');
                        ...
                        searchResults.html('<div class="no-results">Error connecting to server</div>')
                            .removeClass('d-none');
            ...
            function testSearchConnection() {
                $.post('search_notes.php', {
                    ...
                    .done(function(response) {
                        ...
                        alert("Search connection test successful. Check console for details.");
                    })
                    .fail(function(xhr, status, error) {
                        console.error("Search test failed:", status, error);
                        alert("Search connection test failed. Check console for details.");
                    });
            }

            // Add a small button for testing
            $('#searchBox').after('<button type="button" onclick="testSearchConnection()" class="btn btn-sm btn-secondary ms-2">Test Search</button>');
            ...
            // Function to scroll to a note
            function scrollToNote(noteId) {
                const noteElement = $(`#note-${noteId}`);

                if (noteElement.length) {
                    // Scroll to the note
                    $('html, body').animate({
                        scrollTop: noteElement.offset().top - 80
                    }, 500);
